...
text_from_user = st.text_area("Enter the complaint below (a test sample has been shown below)", sample_txt )
...
if st.button("Submit"):
    if text_from_user != "":
        ...
        with col1:
            st.write("**Sklearn Model (Linear SVC) Prediction** : **:green[{}]**".format(output))
            ...
            st.dataframe(
                df_sklearn_model.style.format({"Probability": "{:.3f}"}).apply(highlight_label, axis=1),
        ...
        with col2:
            ...
            st.dataframe(
                df_hugging_face.style.format({"Probability": "{:.3f}"}).apply(highlight_label, axis=1),
                use_container_width=True,
                height=670,
                hide_index=True
            )
        ...
        with st.sidebar:
                st.warning('Distillbert model predictions are more granular since it was trained on a huge data and considered all the 18 labels. For LinearSVM \
                   training, few sub-lables were combined together to form 9 major labels. Example : :red[Student loan, Vehicle loan or lease,\
                   Consumer loan, Payday loan] were combinded together as :red[Loan]', icon="⚠️")
